...
const macdrsi_trade = async (request, response) => {
  ...
  if (trdcount < request.body.tradeCount) {
    setTimeout(async () => {
      ...
      const macd_responce = await macd(
        3,
        10,
        3,
        "close",
      ...
      const macd_val = macd_responce[macd_responce?.length - 1].MACD;
      const signal = macd_responce[macd_responce?.length - 1].signal;
      const prev_macd_val = macd_responce[macd_responce?.length - 2].MACD;
      const prev_signal = macd_responce[macd_responce?.length - 2].signal;
      const kline = stochrsi[stochrsi?.length - 1].k;
      const dline = stochrsi[stochrsi?.length - 1].d;
      const rsi_val = rsi_responce[rsi_responce.length - 1];

      let initialmargin;
      let unrealizedprofit;
      let usdtbalance = 0;

      console.log(macd_val, signal, kline, dline, rsi_val);
      ...
      if (symbolbalance < 0) {
        if (macd_val > signal) {
          if ((0 <= kline && kline <= 30) || (41 <= kline && kline <= 70)) {
            if (kline > dline) {
              if (rsi_val > 50) {
      ...
      if (symbolbalance > 0) {
        if (macd_val < signal) {
          if ((31 <= kline && kline <= 40) || (71 <= kline && kline <= 100)) {
            if (kline < dline) {
              if (rsi_val < 50) {
